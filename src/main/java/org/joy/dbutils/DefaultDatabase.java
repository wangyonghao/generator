...
import org.joy.dbutils.model.ForeignKey;
...
public class DefaultDatabase extends Database {
    ...
    /* (non-Javadoc)
     * @see org.Database#getTable(java.lang.String, java.lang.String, java.lang.String)
     */
    @Override
    ...
    protected void introspectForeignKeys(Table table) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                table.addImportedKey(new ForeignKey(pkTableName, pkColumnName, fkTableName, fkColumnName, iseq));
            }
        } catch (SQLException e) {
            throw e;
        } finally {
    ...
    protected void introspectExportedKeys(Table table) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String seq = rs.getString(KEY_SEQ);
                Integer iseq = new Integer(seq);
                table.addExportedKey(new ForeignKey(pkTableName, pkColumnName, fkTableName, fkColumnName, iseq));
            }
        } catch (SQLException e) {
            throw e;
        } finally {
    ...
    protected void introspectTableComments(Table table, String commentSql, String columnComments) throws SQLException {
        ...
        Map<String,Object> rs = qr.query(connection, commentSql, new MapHandler(), table.getTableName());
        table.setRemarks((String)rs.get(columnComments));
    }
     ...
     *
     * @param table
     * @param columnCommentSql 查询表中列注释的SQL, 含一个占位符 (tableName)
     * @param columnColumnName 字段名列列名
     * @param commentColumnName 注释列列名
     * @throws SQLException
     */
    protected void introspectTableColumnsComments(Table table, String columnCommentSql,String columnColumnName,
                                                  String commentColumnName) throws SQLException {
        QueryRunner qr = new QueryRunner();
        List<Map<String,Object>> rs = qr.query(connection, columnCommentSql, new MapListHandler(), table.getTableName());

        for (Map<String,Object> row : rs){
            String columnName = (String)row.get(columnColumnName);
            String columnComment = (String)row.get(commentColumnName);
